...
case "$mode" in
  ...
  all)
    ...
    "$QA_DIR/lane3-windows-virtual-smoke.sh"
    bash -n "$ROOT/install.sh" "$ROOT/linux/install.sh" "$ROOT/lib/common.sh" "$ROOT"/scripts/*.sh "$ROOT"/scripts/qa/*.sh "$ROOT/config/zshrc.block.sh" "$ROOT/linux/config/zshrc.block.sh"
    check_powershell
    check_ci
