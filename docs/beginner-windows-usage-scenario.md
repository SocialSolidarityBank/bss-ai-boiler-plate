...
> 현재 표준 초보자 설치는 [팀 초보자 표준 설치](./team-beginner-standard-install.md)를 따른다. 이 문서는 Windows 시나리오 참고용이다.
...
- Matt Pocock Skills setup은 사용자가 선택할 수 있게 기능 중심 질문으로 안내한다.
- LazyCodex, oh-my-claudecode, Hermes, Superpowers 같은 add-on은 사용자가 명시적으로 선택할 때만 설치한다.
- Superpowers를 선택하면 초보자 기본값은 Planning Pack(계획 스킬 묶음)인 `brainstorming` + `writing-plans`를 설치하고, 주요 workflow/plugin은 계획 문서에 안내한다.
...
2. GitHub 연결
3. AI CLI 도구 선택
4. 추가 기능 확인: Matt Pocock Skills, Superpowers, Lazy-Codex, Oh-My-Claudecode
5. Final Installation Plan(최종 설치 계획) 발행
6. 승인 뒤 폴더 생성과 설치 실행
```
...
## 5. AI CLI 도구와 Matt Pocock setup

AI 도구 선택은 앱이 아니라 터미널에서 쓰는 `Codex CLI`와 `Claude Code CLI` 기준으로 묻는다.
선택한 CLI 도구만 설치하거나 확인한다.

Matt Pocock Skills는 무엇부터 해야 할지 잘 모를 때 체계적으로 설계하고 작업할 수 있게 도와주는 선택 setup으로 안내한다.
