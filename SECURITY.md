...
BSS AI Helper를 안전하게 써주셔서 고맙습니다. 취약점을 발견하셨다면
...
- **이 키트의 코드**는 범위 안입니다: 마커 블록(`# >>> bss-ai-boilerplate ... >>>`)
  편집, 권한(sudo/관리자) 사용, 다운로드 URL·검증 방식, 제거 스크립트 등.
  여기서 문제를 찾으셨다면 꼭 알려주세요.

참고: 제품 이름은 BSS AI Helper지만 profile 마커는 호환성을 위해
`bss-ai-boilerplate:*` 형식을 계속 씁니다. 취약점 검토에서도 이 마커를 현재
동작으로 보아 주세요.
...
Thanks for helping keep BSS AI Helper safe. Please report vulnerabilities
...
Note: the product name is BSS AI Helper, but profile markers intentionally remain
`bss-ai-boilerplate:*` for compatibility. Treat that marker name as current
behavior during security review.
