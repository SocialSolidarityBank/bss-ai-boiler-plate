...
> 이 문서는 Linux에서 BSS AI Helper가 어떻게 설치되는지 설명합니다. 원본 출처와 보관된 upstream README는 [`../NOTICE`](../NOTICE)와 저장소 루트의 upstream README 스냅샷에서 확인할 수 있습니다.
...
설치가 끝나면 터미널에서 `bss-ai-helper`, `ai-helper`, `bss-ai`를 사용할 수 있습니다. 개발자와 CI는 `--classic`, `--status`, `--dry-run`, `--list`를 계속 사용할 수 있습니다.

> **한국어 빠른 시작** — 터미널을 열고 아래 한 줄을 붙여넣은 뒤 Enter를 누르세요.
> ```sh
> curl -fsSL https://raw.githubusercontent.com/socialsolidaritybank/bss-ai-boiler-plate/main/linux/install.sh | bash
> ```
> apt·dnf·pacman·zypper를 자동 감지합니다. glibc 기반 배포판을 지원하며 Alpine/musl은 지원하지 않습니다. 한국어 전체 안내는 [저장소 메인 README](../README.md#처음-쓰는-분을-위한-안내)를 참고하세요.
...
The piped command bootstraps the repository and then runs the documented
classic installer. To force the question wizard, run `./install.sh --wizard`
from a terminal; if stdin is redirected, the installer tries `/dev/tty` for
answers and falls back to classic mode with a warning when no terminal exists.

Prefer to read before you run (recommended):

```sh
git clone https://github.com/socialsolidaritybank/bss-ai-boiler-plate.git
cd ~/bss-ai-boiler-plate/linux
...
./install.sh --wizard              # run the question wizard; uses /dev/tty when piped
./install.sh --dry-run             # change nothing, just print
./install.sh --yes                 # non-interactive, accept defaults
./install.sh --yes --with-docker   # explicitly allow Docker under --yes
...
Every step is **idempotent** — safe to re-run. `~/.zshrc` is edited only inside
clearly marked managed blocks (`# >>> bss-ai-boilerplate:* >>>`). On re-runs,
those blocks are replaced, not duplicated. Existing files you own are preserved.
...
- **Docker** is opt-in (confirm-gated): the official `get.docker.com` script
  ...
  (effective after re-login). Under `--yes`, Docker is skipped unless you also
  pass `--with-docker` or set `BSS_INSTALL_DOCKER=1` or
